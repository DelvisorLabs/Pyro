...
export async function request(url: URL, method: string, headers: Record<string, string>, body: string | undefined, timeout: number) {
  ...
  try {
    ...
    throw new CliError(`Cannot reach ${url.origin}. This command is using server mode. Use pyro classify --local for standalone checks. For server features, start Docker or set your server URLs, then run pyro doctor --remote. Setup: https://delvisor.com/pyro/docs#setup. Redirects are not followed.`, 4);
  }
