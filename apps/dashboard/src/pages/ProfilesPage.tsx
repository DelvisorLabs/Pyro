...
const invalidControl = "border-red-600 focus:border-red-700 focus:ring-red-700";

function RequiredMark() {
  return <span className="ml-0.5 text-red-700" aria-hidden="true">*</span>;
}

function FieldLabel({ children, help, required = false, htmlFor, className = "", invalid = false }: { children: ReactNode; help?: string; required?: boolean; htmlFor?: string; className?: string; invalid?: boolean }) {
  return <div className="flex items-center gap-1.5"><Label htmlFor={htmlFor} className={`${invalid ? "text-red-700" : ""} ${className}`}>{children}{required && <RequiredMark />}</Label>{help && <HelpTooltip>{help}</HelpTooltip>}</div>;
}
...
function validateProfile(profile: Profile | undefined, profiles: Profile[], serverError?: string): Record<string, string> {
  ...
  if (profile.detectors.length === 0) errors.detectors = "Add at least one detector.";
...
export function ProfilesPage() {
  ...
  const load = () => api.get<{ profiles: Profile[] }>("/api/profiles").then((data) => setProfiles(data.profiles));
  useEffect(() => { void load(); }, []);
  ...
  const save = async () => {
    ...
    setError(undefined);
    try {
      if (isNew) await api.post("/api/profiles", editing);
      else await api.put(`/api/profiles/${editing.id}`, editing);
      await load();
  ...
  return (
    <>
      <PageHeader title="Protection profiles" description="Tune thresholds, failure behavior, limits, and the semantic detectors evaluated in parallel." actions={<Button onClick={createProfile}><Plus className="size-4" />New profile</Button>} />
      <div className="grid gap-3 lg:grid-cols-2">
        {profiles.map((profile) => <Card key={profile.id}>
          <CardHeader className="flex flex-row items-start justify-between gap-4">
            <div><div className="flex items-center gap-2"><CardTitle>{profile.name}</CardTitle>{profile.id === "default" && <span className="border-l border-neutral-300 pl-2 text-[10px] font-semibold uppercase tracking-[0.12em] text-neutral-500">Default</span>}</div><CardDescription>{profile.description || "No description"}</CardDescription></div>
            <div className="flex gap-1"><Button variant="ghost" size="icon" aria-label={`Edit ${profile.name}`} onClick={() => beginEditing(profile)}><Settings2 className="size-4" /></Button>{profile.id !== "default" && <Button variant="ghost" size="icon" aria-label={`Delete ${profile.name}`} onClick={() => void remove(profile.id)}><Trash2 className="size-4" /></Button>}</div>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-3 gap-3 border-b border-neutral-100 pb-4"><div><span className="text-[11px] uppercase tracking-wider text-neutral-500">Review</span><strong className="mt-1 block text-lg">{percent(profile.reviewThreshold, 0)}</strong></div><div><span className="text-[11px] uppercase tracking-wider text-neutral-500">Block</span><strong className="mt-1 block text-lg">{percent(profile.blockThreshold, 0)}</strong></div><div><span className="text-[11px] uppercase tracking-wider text-neutral-500">Detectors</span><strong className="mt-1 block text-lg">{profile.detectors.filter((item) => item.enabled).length}</strong></div></div>
            <div className="mt-4 flex flex-wrap gap-1.5">{profile.detectors.filter((item) => item.enabled).map((item) => <Badge key={item.id} className="normal-case tracking-normal">{item.name}</Badge>)}</div>
          </CardContent>
        </Card>)}
      </div>
      ...
      <Dialog open={Boolean(editing)} onOpenChange={(open) => !open && setEditing(undefined)}>
        <DialogContent className="flex max-h-[90vh] w-[min(900px,calc(100vw-32px))] flex-col overflow-hidden">
          <DialogHeader className="shrink-0 pr-12">
            ...
            <DialogDescription>{isNew ? "Start with an empty profile and add only the detectors and notifications you want." : "Saved changes apply immediately."}</DialogDescription>
          </DialogHeader>
          ...
          {editing && <div className="scrollbar-thin min-h-0 flex-1 space-y-6 overflow-y-scroll px-6 py-5">
            ...
            <div className="flex items-center justify-between gap-4 border border-neutral-200 bg-neutral-50 p-3"><div><div className="flex items-center gap-1.5"><Label>Persist input previews</Label><HelpTooltip>Stores up to 1,000 input characters. Leave off for sensitive traffic.</HelpTooltip></div><p className="mt-0.5 text-xs text-neutral-500">Event metadata and hashes are retained either way.</p></div><Switch checked={editing.persistInputs} onCheckedChange={(checked) => setEditing({ ...editing, persistInputs: checked })} /></div>
            <div className="border border-neutral-200 bg-neutral-50 p-3"><Label>Live notifications</Label><p className="mt-0.5 text-xs text-neutral-500">Choose which actions should be surfaced to connected dashboard users.</p><div className="mt-3 flex gap-5">{(["review", "block"] as const).map((action) => <label className="flex items-center gap-2 text-sm capitalize" key={action}><Switch checked={editing.notifyOn.includes(action)} onCheckedChange={(checked) => setEditing({ ...editing, notifyOn: checked ? [...new Set([...editing.notifyOn, action])] : editing.notifyOn.filter((item) => item !== action) })} />{action}</label>)}</div></div>

            <div>
              <div className="mb-3 flex items-end justify-between gap-3"><div><div className="flex items-center gap-1.5"><Label>Detectors</Label><HelpTooltip>Each detector returns one risk signal for the policy.</HelpTooltip></div><p className="mt-1 text-xs text-neutral-500">Add only the signals this profile should evaluate.</p></div><Button type="button" variant="outline" size="sm" onClick={() => { const index = editing.detectors.length; setEditing({ ...editing, detectors: [...editing.detectors, { id: `custom_${Date.now().toString(36)}`, name: "", description: "", question: "", enabled: true, weight: 1 }] }); setExpandedDetector(index); }}><Plus className="size-3.5" />Add detector</Button></div>
              <div className="space-y-2">
                {editing.detectors.length === 0 && <div className="border border-dashed border-neutral-300 bg-white px-5 py-7 text-center"><p className="text-sm font-medium text-neutral-800">No detectors added</p><p className="mt-1 text-xs text-neutral-500">Add a detector and define the signal this profile should evaluate.</p></div>}
                {editing.detectors.map((detector, index) => {
                  ...
                  return <div className={`border bg-white ${detectorInvalid ? "border-red-600" : "border-neutral-200"}`} key={`${detector.id}-${index}`}>
                    <div className="flex min-h-14 items-center gap-2 px-3">
                      <button type="button" className="flex min-w-0 flex-1 items-center gap-3 py-3 text-left" aria-expanded={expanded} onClick={() => setExpandedDetector(expanded ? null : index)}><ChevronDown className={`size-4 shrink-0 text-neutral-500 transition-transform duration-200 ${expanded ? "rotate-180" : ""}`} /><span className="min-w-0"><strong className="block truncate text-sm font-medium">{detector.name || "Untitled detector"}</strong><span className="mt-0.5 block truncate text-xs text-neutral-500">{detector.description || detector.id}</span></span></button>
                    ...
                    {expanded && <div className="grid gap-4 border-t border-neutral-200 bg-neutral-50/50 p-4">
                      <div className="grid gap-3 sm:grid-cols-2"><div className="space-y-1.5"><FieldLabel help="Shown in decisions and activity." required invalid={Boolean(validation[`${prefix}.name`])} className="text-xs">Name</FieldLabel><Input className={`h-8 ${validation[`${prefix}.name`] ? invalidControl : ""}`} aria-invalid={Boolean(validation[`${prefix}.name`])} value={detector.name} onChange={(event) => updateDetector(index, { name: event.target.value })} /><FieldError message={validation[`${prefix}.name`]} /></div><div className="space-y-1.5"><FieldLabel help="Stable identifier used in API results." required invalid={Boolean(validation[`${prefix}.id`])} className="text-xs">ID</FieldLabel><Input className={`h-8 font-mono text-xs ${validation[`${prefix}.id`] ? invalidControl : ""}`} aria-invalid={Boolean(validation[`${prefix}.id`])} value={detector.id} onChange={(event) => updateDetector(index, { id: event.target.value.toLowerCase().replace(/[^a-z0-9_-]/g, "_") })} /><FieldError message={validation[`${prefix}.id`]} /></div></div>
            ...
            {error && !validation.name && <div className="border border-neutral-500 bg-neutral-100 px-3 py-2 text-sm text-neutral-900">{error}</div>}
          </div>}

          <DialogFooter className="shrink-0 bg-white"><Button variant="outline" onClick={() => setEditing(undefined)}>Cancel</Button><Button onClick={() => void save()} disabled={saving || !formIsValid}>{saving ? "Saving" : isNew ? "Create policy" : "Save policy"}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </>
  );
