...
export function ApiKeysPage() {
  ...
  return (
    <>
      ...
      {!keys.length ? <EmptyState title="No API keys">Create a key before sending traffic to the classification endpoint.</EmptyState> : <Card><Table><TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Application</TableHead><TableHead>Prefix</TableHead><TableHead>Default policy</TableHead><TableHead>Rate limit</TableHead><TableHead>Status</TableHead><TableHead className="w-12" /></TableRow></TableHeader><TableBody>{keys.map((key) => <TableRow key={key.id}><TableCell className="font-medium"><span className="flex items-center gap-2"><KeyRound className="size-4 text-neutral-400" />{key.name}</span></TableCell><TableCell>{apps.find((record) => record.id === (key.appId ?? "default"))?.name ?? key.appId ?? "Default app"}</TableCell><TableCell><code>{key.prefix}…</code></TableCell><TableCell><code>{key.defaultProfileId ?? "app default"}</code></TableCell><TableCell>{key.rateLimitPerMinute ? `${key.rateLimitPerMinute}/min` : "app default"}</TableCell><TableCell>{key.revokedAt ? <Badge className="border-neutral-300 text-neutral-500">revoked</Badge> : <Badge className="border-neutral-950 bg-neutral-950 text-white">active</Badge>}</TableCell><TableCell>{!key.revokedAt && <Button variant="ghost" size="icon" onClick={() => void revoke(key.id)} aria-label={`Revoke ${key.name}`}><Trash2 className="size-4" /></Button>}</TableCell></TableRow>)}</TableBody></Table></Card>}
